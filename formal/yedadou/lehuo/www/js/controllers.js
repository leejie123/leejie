...
.controller('AppCtrl', function($scope, $http, $ionicModal, $location, $ionicLoading, $ionicActionSheet, $timeout, $ionicPopup, $ionicScrollDelegate) {
  ...
  getTopiceUrl = ROOT_PATH + 'moreTopice.json';
...
.controller('topice', function($scope , Book, $http, $stateParams, $ionicSlideBoxDelegate){
    $scope.next = function() {
    ...
    $scope.loadMore = function() {
      ...
      // })
      $http.get(getTopiceUrl).success(function(data) {
        var xx = [
          {
            "image": "http://placehold.it/200x200",
            "title": "这个wewew时代，被电脑占据着，快没有灵气了。",
            "month": "Novemver",
            "day": "19",
            "link": "#app/Detail/12",
            "id": 13
        ...
        $scope.topice = $scope.topice.concat(data);
        // console.log($scope.topice);
        // $scope.noMoreItemsAvailavle = true;
      }).error(function(data) {
        alert('网络超时')
      })
    }
