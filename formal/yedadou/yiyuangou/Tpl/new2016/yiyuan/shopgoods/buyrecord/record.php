...
<div class="user" style="width:auto;padding:18px;background:url({__STATIC_URL__}/public/yiyuan/new2016/images/pic_bg.png) no-repeat;background-size:cover;background-position:center;">
	<?php
		$avatar = '{__STATIC_URL__}/public/images/default_user.gif'; 
		if(!empty($buyer['avatar'])) {
			preg_match('/http\:\/\/ydd.img.yedadou.com/', val($buyer,'avatar'), $pic);
			if(!empty($pic)) {
				$avatar = $buyer['avatar'];
			}else{
				$avatar = substr($buyer['avatar'],0,strlen($buyer['avatar'])-1).'46';
			}
		}
	?>
	<img src="<?=$avatar?>" alt="" style='border:0px;'>
	<div class="name" style="margin-top:18px;">
		<span style="font-size:18px;width:100px;overflow:hidden;display:block;margin-bottom:8px;"><?=$buyer['nick'];?></span>
		<span style="font-size:14px;">ID:<?=$buyer['uid'];?></span></div>
</div>
