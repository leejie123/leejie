				...
				<div class="col-xs-12">
					...
					<div class="tabbable">
						<ul class="nav nav-tabs padding-12 tab-color-blue">
							<li class="active">
								<a data-toggle="tab" href="#tab1">订单列表</a>
							</li>
						</ul>
						...
						<div class="tab-content no-border padding-0 pt10">
							<div id="tab1" class="tab-pane active">
								...
								<div class="table-head-toolbar clearfix">
									<form method="get" action="/yiyuan/admin/order/orderList/get">
										<div class="pull-right clearfix">
											...
											<div class="pull-left span-label" id="returnShowLabel">购买时间:</div>
										...
										<a href="#" id="purview" class="btn btn-info btn-sm" data-url="/yiyuan/admin/order/exportOrder/get?isExport=1&orderId=<?=val($search,'order_sn','')?>&userCount=<?=val($search,'userCount','')?>&goodsCount=<?=val($search,'goodsCount','')?>&startTime=<?=val($search,'startTime','')?>&endTime=<?=val($search,'endTime','')?>">导出</a>
								</div>
								...
								<table id="dynamic-table" class="table table-striped table-bordered table-hover table-vertical-middle">
									...
									<tbody>
										...
										<?php foreach ($data as $key=>$value):?>
											...
											<td class="center" valign="middle"><?=$value['buyer_name']?>&nbsp;(<?=$value['buyer_uid']?>)</td>
											<td class="center blue" valign="middle"><?=$value['quantity']?></td>
											<td class="center" valign="middle">￥<?=$value['order_amount']?></td>
											<td class="center" valign="middle">￥<?=$value['amount_paid']?></td>
											<td class="center" valign="middle"><?=val($value,'use_point',0)?></td>
											<td class="center"><?=date('Y-m-d H:i:s',$value['create_time']) ?></td>
											<td class="center"><?=val($value, 'hit', 0) == 1? '<span class="red">是</span>':'否'?></td>
											<td class="center"><?=val($value, 'hit', 0) == 1 && $value['status'] == 2 ? $value['status_name'].' 待发货': $value['status_name']?></td>
											<td class="center">
												<a href="/yiyuan/admin/order/OrderList/get?act=detail&id=<?php echo $value['buyer_uid'];?>&term_id=<?php echo $value['term_id'];?>&order_sn=<?=$value['order_sn'];?>" class="blue" data-id="edit">查看详细</a>
											</td>
										</tr>
										<?php endforeach;?>
										<?php }?>
