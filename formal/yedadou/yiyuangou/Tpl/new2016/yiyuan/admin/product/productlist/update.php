				...
				<div class="col-xs-12">
					...
					<div class="tabbable">
						<ul class="nav nav-tabs padding-12 tab-color-blue">
							...
							<li class="active">
								<a data-toggle="tab" href="#tab1"><i class="ace-icon fa fa-plus"></i> 编辑商品</a>
							</li>
						</ul>

						<div class="tab-content no-border padding-0 pt10">
							<div id="tab1" class="tab-pane active">
								<form method="post" action="/yiyuan/admin/product/productList/update?act=update&id=<?=$data['id']?>">
											...
											<div id="t1" class="tab-pane active clearfix">
												<div class="form-group">
													<div class="label-wrapper">
														<label class="" for=""><span class="font-red">说明：</span>	</label>
													</div>
													<div class="control-wrapper short">													
														<label>
															<span class="font-red">进行中的商品，价格等属性不可被修改，如需修改请改变最大期数，待结束后去商品回收站中编辑。
															</span>		
														</label>																																										
													</div>
												</div>
													...
													<div class="control-wrapper short">
														...
														<label>
															<select id="tag_recommend" name="tag_recommend">
																...
																<?php for($i=1; $i<=20;$i++){?>
																<option value="<?=$i?>" <?php if($data['tag_recommend']==$i){?>selected<?php }?> ><?php echo "推荐".$i;?></option>
																<?php } ?>
															</select>
															&nbsp;
														</label>
														...
														<label>
															<select id="tag_popular" name="tag_popular">
																<option value="0" selected>无</option>
																<?php for($i=1; $i<=20;$i++){?>
																<option value="<?=$i?>" <?php if($data['tag_popular']==$i){?>selected<?php }?> ><?php echo "人气".$i;?></option>
																<?php } ?>
															</select>
															&nbsp;
														</label>
														...
														<label>
															<select id="tag_new" name="tag_new">
																<option value="0" selected>无</option>
																<?php for($i=1; $i<=20;$i++){?>
																<option value="<?=$i?>" <?php if($data['tag_new']==$i){?>selected<?php }?> ><?php echo "新品".$i;?></option>
																<?php } ?>
															</select>
															&nbsp;
														</label>
														...
														<label>
															<select id="tag_stocking" name="tag_stocking">
																<option value="0" selected>无</option>
																<?php for($i=1; $i<=20;$i++){?>
																<option value="<?=$i?>" <?php if($data['tag_stocking']==$i){?>selected<?php }?> ><?php echo "年货".$i;?></option>
																<?php } ?>
															</select>
															&nbsp;
														</label>
												...
												<div class="form-group">
													...
													<div class="control-wrapper short">
														<div class="input-group ">
															<input type="text" value="<?=$data['price']?>" name="price" id="price" class="form-control" placeholder="" <?php if($status==1){echo 'readonly="readonly"';} ?>>
												...
												<div class="form-group">
													...
													<div class="control-wrapper short">
														<div class="input-group ">
															<input type="text" value="<?=$data['code_price']?>" name="code_price" id="oncePrice" class="form-control" placeholder="" <?php if($status==1){echo 'readonly="readonly"';} ?>>
															<span class="input-group-addon">元</span>
														</div>
													</div>
												</div>
												...
												<div class="form-group">
													<div class="label-wrapper">
														...
														<span class="font-red" style="float: left;">(单张&lt;2M)<br>尺寸为宽640x高360,注：宽度可以大一些加背景适应不同宽度的手机屏幕。</span>
														</label>
													</div>
													<div class="control-wrapper ">
														...
														<?php endforeach;?>
														<?php }else{?>
															<img id="previewImage<?php echo $key+1;?>" class="preview-image" src="<?=val($data,'pic_url','{__STATIC_URL__}/public/admin/images/noupload.png') ?>" onerror="this.src='{__STATIC_URL__}/public/admin/images/noupload.png'">
															<input type="hidden" name="img_list[]" id="imageUrl<?php echo $key+1;?>" value="<?=val($data,'pic_url','{__STATIC_URL__}/public/admin/images/noupload.png') ?>" />
														<?php }?>
														</ul>
													</div>
												</div>
												...
												<div class="form-group">
													...
													<div class="control-wrapper short">
														<div  class="time-container ">
															<input id="bTime" name="begin_time" type="text" class="" value="<?=!empty($data['begin_time'])?date('Y-m-d H:i:s',$data['begin_time']):''?>" <?php if($status==1){echo 'readonly="readonly"';} ?>>
														</div>
												...
												<div class="form-group">
													...
													<div class="control-wrapper short">
														<label>
															...
															<input name="is_interval" type="radio" class="ace" value="0" <?php if($status==1){echo 'readonly="readonly"';} ?>>
															<?php }else{?>
															<input name="is_interval" type="radio" class="ace" value="0" checked="checked" <?php if($status==1){echo 'readonly="readonly"';} ?>>
															<?php }?>
													...
													<div class="control-wrapper short">
														<div  class="time-container ">
															<label>
																<?php if(!empty($data['interval_time'])){?>
																<input name="is_interval" type="radio" class="ace" value="1" checked="checked" <?php if($status==1){echo 'readonly="readonly"';} ?>>
																<?php }else{?>
																<input name="is_interval" type="radio" class="ace" value="1" <?php if($status==1){echo 'readonly="readonly"';} ?>>
																<?php }?>
																<span class="lbl"> 限时揭晓</span>
															</label>
															&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
															<label class="" for="iTime">揭晓时间：</label>
															<input id="iTime" name="interval_time" type="text" class="" value="<?=$data['interval_time']?>" <?php if($status==1){echo 'readonly="readonly"';} ?>>
														</div>
												...
												<div class="form-group">
													...
													<div class="control-wrapper long">
														...
														<?=$productYiyuanGoodsConfig?htmlspecialchars_decode($productYiyuanGoodsConfig['desc']):'';?></textarea>
													</div>
												</div>
