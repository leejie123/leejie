				...
				<div class="col-xs-12">
					...
					<div class="tabbable">
						<ul class="nav nav-tabs padding-12 tab-color-blue">
							<li class="active">
								<a data-toggle="tab" href="#tab1">商品列表</a>
							</li>
						</ul>
						...
						<div class="tab-content no-border padding-0 pt10">
							<div id="tab1" class="tab-pane active">
								...
								<div class="table-head-toolbar clearfix">
									<form method="get" action="/yiyuan/admin/product/productList">
...
<!--                                    导出商品信息-->
                                    <form action="/Yiyuan/Admin/Product/ProductList/post" method="post">
                                        <!--已经被选中的id,以逗号分隔,每次操作复选框时会自动更新该控件的值-->
                                        <input type="hidden" name="checkedIds" id="checkedIds" value="" />
										<input class="btn btn-success btn-sm" type="submit" style="width: 60px;" id="export" name="导出" value="导出"/>
                                    </form>

                                    <a class="btn btn-success btn-sm" id="id-add" href="/yiyuan/admin/product/productList/add"><i class="ace-icon fa fa-plus"></i> 添加商品</a>
<!--										<a class="btn btn-success btn-sm" id="id-add" href="/yiyuan/admin/product/productList/add">导入商城商品</a>	-->
								</div>
									...
									<tbody>
										...
										<?php foreach ($products as $key => $value):?>
											...
											<?php 
												...
												if($value['tag_new']>0) echo '【新品】</br>';
												if($value['tag_stocking']>0) echo '【年货】';
											?>
											</td>
											<td class="center" valign="middle"><?php echo val($value,'sort_order',1);?></td>
											<td class="center"><span class="red"><?php echo $value['buy_num'];?></span>/<?php echo $value['code_num'];?></td>
											<td class="center"><?php echo $value['code_price'];?></td>
											<td class="center"><?php echo $value['over_term'];?>/<?php echo $value['term_num'];?></td>
											<td class="center"><?php echo val($value,'status_name','');?></td>
											<td class="center"> 
												<!-- add	 -->
												<!-- 重新开团页面和加入拼团页面一样 -->
												<!-- <a href="http://1047.m.yedadou.com/shop/test/index?tplName=pintuan">重新开团</a> -->
												<!-- <a href="http://1047.m.yedadou.com/shop/test/index?tplName=pintuanrecord">拼团记录</a> -->
												<!-- add	 -->
												<a href="/yiyuan/admin/product/productList/update?act=update&id=<?php echo isset($value['id'])?$value['id']:''; ?>" class="blue" data-id="edit">编辑</a>
												<a href="/yiyuan/pintuan/admin/product/addProduct?goods_id=<?php echo isset($value['id'])?$value['id']:''; ?>" >加入拼团</a>
												</br>
												<a href="#" data-url="/yiyuan/admin/product/productList/update?act=del&id=<?php echo isset($value['id'])?$value['id']:''; ?>" class="blue" data-id="delete">删除</a>
    ...
    <script>
        ...
        var editUrl = ""; //编辑备注的网址
        var synchronousUrl = "/Sync/FansSync"; //同步粉丝的网址
        var deleteUrl = ""; //删除的地址
        var getAppsUrl = "/app/getList"; //获取应用列表
        var superPartnerUrl = "/UserCenter/Fans/post";
        var upgradeToAgentUrl = "/Agent/Fans?act=upgradeToAgent"; //升级代理的网址
        var manageUrl = "/Agent/Fans?act=manage"; //管理的网址
