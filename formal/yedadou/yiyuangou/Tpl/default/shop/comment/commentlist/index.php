    ...
    <body class="">
        <div class="wrapper">
            ...
            <div style="padding: 10px;">
                <div class="head-code">
                    <div class="dingdan">订单编号：<?php echo val($order, 'order_sn'); ?></div>
                    <div style="border-bottom: 1px solid #e8e8e8;text-align:center;"><?php echo date('Y-m-d H:i:s', $order['create_time']); ?></div>
                </div>
                ...
                <?php foreach (val($order, 'items', []) as $val):; ?>
                <div class="card">
                    <div class="item">
                        <div class="item-media">
                            <a href="">
                                <div style="background-image: url('<?php echo val($val, 'pic_url', 'http://placehold.it/200x200'); ?>');width:100%;height:100%;"></div>
                            </a>
                        </div>
                        <div class="item-content">
                            <div  class="mg-b-5">
                                <span style="display:block;margin-bottom: 5px;" class="font-16"><?php echo val($val, 'title'); ?></span> 
                                <span style="display:block;margin-bottom:10px;" class="font-12">
                                    <em class="color-orange style-normal">数量：<?php echo val($val, 'quantity'); ?>件</em>
                                </span> 
                                <span class="font-12 color-orange bold">价格：￥<?php echo val($val, 'price'); ?></span>
                            </div>
                        </div>
                        <?php if (!in_array($val['goods_id'], $already)):; ?>
                            <a href="/Shop/Comment/DoComment?orderSn=<?php echo val($order, 'order_sn'); ?>&goodsId=<?php echo val($val, 'goods_id'); ?>&title=<?php echo val($val, 'title'); ?>">去评价</a>
                        <?php else:?>
                            已评价
                        <?php endif; ?>
                    </div>
