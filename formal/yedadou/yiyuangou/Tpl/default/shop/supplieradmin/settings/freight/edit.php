				...
				<div class="col-xs-12">
					...
					<div class="tabbable">
						...
						<form method="post" action="" enctype="multipart/form-data" data-submit="auto">
								...
								<div id="tab1" class="tab-pane active">
									...
									<div class="form-group">
										...
										<div class="control-wrapper">
											<input type="text" id="freightName" name="freightName" class="form-control" value="<?= !empty($info) ? $info['name'] : '';?>" placeholder="请输入名称">
										</div>
									</div>
									<div class="form-group">
										<div class="label-wrapper">
									...
									<div class="form-group">
										...
										<div class="control-wrapper">
											<select id="countType" name="countType" class="form-control">
												<option value="1" <?php if(!empty($info)) {echo $info['type'] == '1' ? 'selected' : '';} ?>>固定运费</option>
												<option value="2" <?php if(!empty($info)) {echo $info['type'] == '2' ? 'selected' : '';} ?>>按件计费</option>
											</select>
										</div>
									</div>
										...
										<div class="form-group">
											...
											<div class="control-wrapper">
												<div class="input-group">
													<input type="text" value="<?= (!empty($info) && $info['config']['price']['fixMoney'] > 0) ? $info['config']['price']['fixMoney'] : '';?>" name="fixMoney" class="form-control"><span class="input-group-addon">元</span>
												</div>
											</div>
										</div>
									</div>
													...
													<tbody>
														<tr>
															...
															<?php 
																$firstNum = '';
																$firstPirce = '';
																if (!empty($info['config']['price']['pieceFirst'])) {
																	$firstNum = $info['config']['price']['pieceFirst'][0];
																	$firstPirce = $info['config']['price']['pieceFirst'][1];
																}
															?>
															<td class="center">
																<div class="input-group">
																		<input type="text" value="<?= $firstNum; ?>" name="pieceFirst[]" class="form-control"><span class="input-group-addon">件</span>
																	</div>
															</td>
															<td class="center">
																<div class="input-group">
																	<input type="text" value="<?= $firstPirce; ?>" name="pieceFirst[]" class="form-control"><span class="input-group-addon">元</span>
																</div>
															</td>
														</tr>
														<tr>
															<?php
																$otherNum = '';
																$otherPirce = '';
																if (!empty($info['config']['price']['pieceOther'])) {
																	$otherNum = $info['config']['price']['pieceOther'][0];
																	$otherPirce = $info['config']['price']['pieceOther'][1];
																}
															?>
															...
															<td class="center">
																<div class="input-group">
																	<input type="text" value="<?= $otherNum; ?>" name="pieceOther[]" class="form-control"><span class="input-group-addon">件</span>
																</div>
															</td>
															<td class="center">
																<div class="input-group">
																	<input type="text" value="<?= $otherPirce; ?>" name="pieceOther[]" class="form-control"><span class="input-group-addon">元</span>
																</div>
															</td>
														</tr>
													</tbody>
							...
							<button class="btn btn-info" id="submitBtn" name="submitBtn" type="submit"  data-href="/shop/supplierAdmin/settings/freight/update" data-action="submit" data-refresh="window" data-mask="parent" ><i class="ace-icon fa fa-save"></i> 保存</button>
							<div class="space"></div>
						</form>
					</div>
					<!--内容结束-->
