...
<html>
	...
	<body class="no-skin">
											...
											<div id="t1" class="tab-pane active clearfix">
												...
												<div class="form-group">
													...
													<div class="control-wrapper short">
														...
														<input type="hidden" name="subCate_id"  value="2">
														<select id="category1" name="category1" data-tip="请选择一级分类">
															<option  value="">请选择一级分类</option>
															<?php foreach ($categorys as $key=>$value){?>
																<option value="<?=val($value,'name')?>" data-id="440000"><?=val($value,'name')?></option>
															<?php }?>
														</select>
														&nbsp;
		...
		<script>
		var data = <?=$data?>;
		var category = <?=json_encode($categorys)?>;
		var tableId = "#parameterTable"; //表格ID
		var subCategory_property = <?=json_encode($subCategory_property)?>;
		  ...
		  $('#category2').on('change', function() {
  			...
  			if (subCategory_property) {
  				// var content = $.parseJSON(subCategory_property);
  				var content = subCategory_property;
				var result = false;	
  				...
  				$(content).each(function(key, value) {
				  	if (value.name == title) {
				  		...
				  		$(value.property).each(function(k, v){
				  		...
				  		$('<td><input type="textarea" value="" name="" class="form-control" placeholder=""></td>').appendTo(tr);
