								...
								<table id="dynamic-table" class="table table-striped table-bordered table-hover table-vertical-middle">
									...
									<tbody>
										...
										<?php foreach ($data as $key=>$value):?>
											...
											<td class="center" valign="middle"><?=$value['buyer_nick']?>&nbsp;(<?=$value['buyer_id']?>)</td>
											<td class="center blue" valign="middle"><?=$value['quantity']?></td>
											<td class="center" valign="middle">￥<?=$value['order_amount'] + val($value,'discount',0)?></td>
											...
											<td class="center" style=" color:red;">中奖</td>
											<td class="center"><?=val($value,'status_name','已支付 待发货')?></td>
											<td class="center">
												<a href="/yiyuan/admin/order/OrderList/get?act=detail&id=<?php echo $value['buyer_id'];?>&term_id=<?php echo $value['term_id'];?>&order_sn=<?=$value['order_sn'];?>" class="blue" data-id="edit">查看详细</a>
											</td>
										</tr>
										<?php endforeach;?>
										<?php }?>
