	...
	<div class="proItem">
		<div class="row clearfix">
			...
			<?php if($v['code_price']>1&&$v['code_price']<11) {?>
					<div class="tipTag" style="background-image: url(http://static.yedadou.com/public/yiyuan/images/tip<?=floor($v['code_price'])?>.png)"></div>
				<?php } ?>
			</div>
			<div class="proleft">
				<div onclick="window.location.href='/yiyuan/ShopGoods/ProductDetails?id=<?=$v['id'];?>';" class="row title"><?php echo val($v,'title', '');?></div>
				<div class="row clearfix">
					...
					<div class="progressContainer clearfix">
						<div class="progress">
						<?php
							$num = val($v,'code_price', 1) >0 ? val($v, 'price', 0) / val($v, 'code_price') : 0;
							$proc = $num > 0 ? round(val($v, 'buy_num', 0) / $num * 100) : 0;
						?>
					      <div class="progress-bar" data-to="<?= $proc ?>"></div>
					    </div>
					    <div class="states">
					    	<div class="states-left">总需<?php echo val($v, 'all_num', 0);?>人次</div>
