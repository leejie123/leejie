...
class NBDTWechat
{
	...
	const CARD_LOCATION_GET               = '/cgi-bin/poi/getpoi?';
	...
	// 门店管理相关地址
	const POI_CREATE                      = '/cgi-bin/poi/addpoi?';
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	// 获取卡券cardExt
	public function getCardPackage($cardId, $code, $timestamp = ''){
            if(!empty($timestamp)){
                $timestamp = $timestamp;
            }
            else{
            ...
            $resultArray['cardExt']['code'] = $code;
            $resultArray['cardExt']['openid'] = '';
            $resultArray['cardExt']['timestamp'] = $arrays['timestamp'];
            $resultArray['cardExt']['signature'] = $string;
            

            $temp;
            foreach ($resultArray['cardExt'] as $key => $value) {
            	if($value != ''){
            		$temp[$key] = $value;
            	}
            }
            $resultArray['cardExtJson'] = json_encode($temp);
            $resultArray['cardExtCombo'] = $temp;
            // print_r($arrays);
            return $resultArray;
    }
    ...
    public function getCardLocation($poiid){
    	if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_LOCATION_BATCHGET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
     ...
     * }
     */
    public function checkCardCode($code, $cardId) {
        $data = array(
            'code' => $code,
        );
