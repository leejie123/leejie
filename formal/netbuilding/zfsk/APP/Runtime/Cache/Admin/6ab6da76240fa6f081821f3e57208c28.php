 ...
 <!-- |---------------------------------------------------------| -->
  <form class="form-horizontal">
        <div class="form-group">
          ...
          <div class="col-md-8">
            <a href="<?php echo U('My/addUser');?>" class="btn btn-success">添加会员</a>
          </div>
        </div>
       <table>
          <table class="table table-striped table-responsive datatable">
                ...
                <tbody>
                  <?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo (searchdict($user["status"],"USER_STATUS","value")); ?></td>
                        <td><?php echo ($user["remarks"]); ?></td>
                        <td>
                        <a href="<?php echo U('My/shenheuser',array('id' =>$user['id']));?>">修改会员身份</a><span>/</span><a href="<?php echo U('My/updateUser',array('id' =>$user['id']));?>">修改</a><span>/</span><a href="<?php echo U('My/deleteUser',array('id' =>$user['id']));?>">删除</a></td>
