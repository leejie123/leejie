...
<!--           <form action="#" method="get" class="sidebar-form">
 ...
 <!-- |---------------------------------------------------------| -->
<form class="form-horizontal" action="<?php echo U('Rbac/setAccess');?>" method="post" enctype="multipart/form-data">
<fieldset>
    <td><div style="padding-left:20px;">你正在为用户组：<span style="font-weight:bold;font-size:15px;color:blue;"><?php echo ($name); ?></span> 分配权限。</div>
    <?php if(is_array($nodelist)): $i = 0; $__LIST__ = $nodelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><p style="text-indent:<?php echo ($vo['level']*20); ?>px; <?php if($vo["level"] == 3): ?>float:left<?php else: ?>clear:both;<?php endif; ?>"> 
             ...
             <input id="<?php echo ($vo["id"]); ?>" type="checkbox" name="access[]" value="<?php echo ($vo["id"]); ?>_<?php echo ($vo["level"]); ?>" pid="<?php echo ($vo["pid"]); ?>" level="<?php echo ($vo["level"]); ?>" <?php if($vo['access']): ?>checked="checked"<?php endif; ?> />
              <?php if($vo['level'] == '1'): ?><span style="font-weight:bold;">[项目]</span><?php endif; ?>
              <?php if($vo['level'] == '2'): ?><span style="font-weight:bold;">[模块]</span><?php endif; ?>
              <label name="name" value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></label>
