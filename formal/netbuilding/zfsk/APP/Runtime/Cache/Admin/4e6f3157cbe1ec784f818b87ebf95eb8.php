      ...
      <div class="form-group">
       ...
       <table>
          <table class="table table-striped table-responsive datatable">
                ...
                <tbody>
                <?php if(is_array($status)): $i = 0; $__LIST__ = $status;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$status): $mod = ($i % 2 );++$i;?><tr>
                      <!-- <td><?php echo ($real["id"]); ?></td> -->
                      <td><?php echo ($status["key"]); ?></td>
                      <td><?php echo (displaydateifnotnull($status["request_timestamp"],'m/d H:i')); ?></td>
                      <td><?php echo (searchdict($status["table"],"APPROVE_TYPE","value")); ?></td>
                      <td><?php echo ($status["approve_user_id"]); ?></td>
                      <td><?php echo (displaydateifnotnull($status["approve_timestamp"],'m/d H:i')); ?></td>
                      <td><?php echo ($status["request"]); ?></td>  
