      ...
      <div class="form-group">
       ...
       <table>
          <table class="table table-striped table-responsive datatable">
                ...
                <tbody>
                <?php if(is_array($shenhe)): $i = 0; $__LIST__ = $shenhe;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$shenhe): $mod = ($i % 2 );++$i;?><tr>
                      <!-- <td><?php echo ($real["id"]); ?></td> -->
                      <td><?php echo ($shenhe["staff"]["truename"]); ?></td>
                      <td><?php echo (displaydateifnotnull($shenhe["request_timestamp"],'m/d H:i')); ?></td>
                      <td><?php echo ($shenhe["type"]); ?></td>
                      <td><?php echo ($shenhe["request"]); ?></td>
                      <td data-status="<?php echo ($shenhe["is_approved"]); ?>"><?php echo (searchdict($shenhe["is_approved"],"IS_APPROVED","value")); ?></td>
                      <td><?php echo (displaydateifnotnull($shenhe["approve_timestamp"],'m/d H:i')); ?></td>
                      <td><?php echo ($shenhe['notes']); ?></td>
                      <td><?php echo ($shenhe['remarks']); ?></td>
                      <td>
                        <?php if(($shenhe['is_approved']) == "0"): ?><a href="<?php echo U('My/approve',array('id' =>$shenhe['id'], 'is_approve' => 1));?>">通过审核</a><span>/</span>
                          <a href="<?php echo U('My/approve',array('id' =>$shenhe['id'], 'is_approve' => 0));?>">拒绝审核</a><?php endif; ?>
...
<!--  |---------------------------------------------------------| -->
    ...
    <script type="text/javascript">
      $(document).ready(function() {
        ...
        $('td[data-status]').each(function(k, v){
          $v = $(v);
          if($v.data('status') == -1){
            $v.css('color', 'red');  
          }else if($v.data('status') == 1){
            $v.css('color', 'green');  
          }
