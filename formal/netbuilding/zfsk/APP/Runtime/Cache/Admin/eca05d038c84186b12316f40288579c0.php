       ...
       <table>
              <table class="table table-striped table-responsive datatable">
                    ...
                    <?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo (date('Y-m-d H:i',$vo["logintime"])); ?></td>
                        <td><?php echo ($vo["loginip"]); ?></td>
                        <td><?php echo ($vo['role'][0]['name']); ?></td>
                        <td><a href="<?php echo U('Rbac/saveUserHandle',array('id' =>$vo['id']));?>">更改权限</a><span> / </span><a href="<?php echo U('Rbac/updateStaff',array('id' =>$vo['id']));?>">修改</a><span> / </span><a class="confirm-delete" href="<?php echo U('Rbac/deleteuser',array('id' =>$vo['id']));?>">删除</a></td>
