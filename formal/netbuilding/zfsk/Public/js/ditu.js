...
qq.maps.event.addListener(map, "click", function (e) {
    ...
    url3 = encodeURI("http://apis.map.qq.com/ws/geocoder/v1/?location=" + e.latLng.getLat() + "," + e.latLng.getLng() + "&key=YPZBZ-VMCRJ-XLPF7-FXYDH-URYNJ-FXBKQ&output=jsonp&&callback=?");
...
qq.maps.event.addDomListener(btnSearch, 'click', function () {
    ...
    url = encodeURI("http://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + query_city + ",0)&page_size=9&page_index=1&key=YPZBZ-VMCRJ-XLPF7-FXYDH-URYNJ-FXBKQ&output=jsonp&&callback=?");
    $.getJSON(url, function (result) {
        ...
        if (result.count) {
            ...
            each(result.data, function (n, ele) {
                ...
                var leftn = -54 - 17 * n;
                order.style.cssText = "width:17px;height:17px;margin:3px 3px 0px 0px;float:left;background:url({iMicms::STATICS}/tencentmap//img/marker_n.png); " + leftn + "px 0px";
                div.appendChild(order);
                var pannel = document.createElement("div");
                pannel.style.cssText = "width:84%;float:left;";
...
function setAnchor(marker, flag) {
    ...
    if (flag == true) {
        var anchor = new qq.maps.Point(10, 30),
                ...
                icon = new qq.maps.MarkerImage("{iMicms::STATICS}/tencentmap/img/marker10.png", size, origin, anchor);
        marker.setIcon(icon);
    } else {
        var anchor = new qq.maps.Point(10, 30),
...
city.onclick = function (e) {
    ...
    if (target.className == "city_name") {
        ...
        url2 = encodeURI("http://apis.map.qq.com/ws/geocoder/v1/?region=" + curCity.children[0].innerHTML + "&address=" + curCity.children[0].innerHTML + "&key=YPZBZ-VMCRJ-XLPF7-FXYDH-URYNJ-FXBKQ&output=jsonp&&callback=?");
...
$(".search_t").autocomplete({
    source:function(request,response){
        url4 = encodeURI("http://apis.map.qq.com/ws/place/v1/suggestion/?keyword=" + request.term + "&region=" + curCity.children[0].innerHTML + "&key=YPZBZ-VMCRJ-XLPF7-FXYDH-URYNJ-FXBKQ&output=jsonp&&callback=?");
